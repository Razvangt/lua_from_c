...
local function is_in_bounds(x, y)
	return (1 <= x and x <= grid_w and 1 <= y and y <= grid_h)
end
...
local function drill_path_from(x, y)
	...
	while #neighbor_directions > 0 do
		local direction = table.remove(neighbor_directions, math.random(#neighbor_directions))
		grid[x + direction[1]][y + direction[2]] = "open"
		drill_path_from(x + 2 * direction[1], y + 2 * direction[2])
		neighbor_directions = get_neighbor_directions(x, y, percent_extra_paths)
	end
end
